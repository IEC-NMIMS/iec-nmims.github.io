...
export default function DomeGallery({
  ...
  /** Hide tiles whose face points away from camera (>110° off-axis) */
  const cullHiddenTiles = useCallback(
    (sphereYDeg: number) => {
      const sphere = sphereRef.current;
      if (!sphere) return;
      const unit = 360 / segments / 2;
      const children = sphere.children;
      for (let i = 0; i < children.length; i++) {
        const child = children[i] as HTMLElement;
        const ox = parseFloat(child.dataset.offsetX || "0");
        const sx = parseFloat(child.dataset.sizeX || "2");
        const tileY = unit * (ox + (sx - 1) / 2);
        const effective = wrapAngleSigned(tileY + sphereYDeg);
        child.style.display = Math.abs(effective) > 110 ? "none" : "";
      }
    },
    [segments]
  );
  ...
  const applyTransform = (xDeg: number, yDeg: number) => {
    ...
    // Throttled visibility culling — run at most every 120ms
    const now = performance.now();
    if (now - lastCullRef.current > 120) {
      lastCullRef.current = now;
      cullHiddenTiles(yDeg);
    } else if (!cullRAFRef.current) {
      cullRAFRef.current = requestAnimationFrame(() => {
        cullRAFRef.current = null;
        lastCullRef.current = performance.now();
        cullHiddenTiles(rotationRef.current.y);
      });
    }
  };
